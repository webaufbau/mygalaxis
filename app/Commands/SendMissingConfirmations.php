...
class SendMissingConfirmations extends BaseCommand
{
    ...
    public function run(array $params)
    {
        ...
        // Gruppiere nach Email und dann nach group_id
        $groupedByEmail = [];
        foreach ($offersWithoutConfirmation as $offer) {
            ...
            if (!isset($groupedByEmail[$email][$groupKey])) {
                $groupedByEmail[$email][$groupKey] = [];
            }

            $groupedByEmail[$email][$groupKey][] = $offer;
        }

        // Verarbeite jede Email-Gruppe
        $totalSent = 0;
        foreach ($groupedByEmail as $email => $groups) {
            ...
            foreach ($groups as $groupKey => $offers) {
                if (count($offers) === 1) {
                    ...
                    // Mehrere Angebote in Gruppe
                    $this->sendGroupedConfirmation($offers);
    ...
    private function sendSingleConfirmation(array $offer): void
    {
        ...
        $email->setBCC($this->siteConfig->email);
    ...
    private function sendGroupedConfirmation(array $offers): void
    {
